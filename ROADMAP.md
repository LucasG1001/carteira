...
Funcionalidades planejadas para Investimentos e Gastos. Implementadas uma por vez, com commit ao final de cada.

**Navegação:** sub-abas no topo de cada seção.
- Investimentos → `Carteira · Proventos · Lançamentos · Imposto de Renda`
- Gastos → `Visão geral` (+ `Assinaturas · Parcelas` conforme forem entrando)
...
- **Evolução do Patrimônio** — na Carteira, gráfico de barras com o **aporte acumulado** (compras − vendas) mês a mês. Endpoint `GET /portfolio/evolution`. *(sem cotação histórica; ver observação abaixo)*
