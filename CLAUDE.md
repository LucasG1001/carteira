...
- **`pages/ExpensesPage/`** — `ExpensesLayout` (envolve `ExpensesProvider` ou `GoalsProvider` conforme a rota) + `ExpensesPage` (gastos do mês: cards, breakdown e tabela filtrável) e `pages/GoalsPage/` (caixinhas). Resumo, "travado x escolha sua" e compromissos são sempre o mês corrente; o "ritmo mês a mês" tem seu `PeriodFilter` (janela deslizante). Há um único `MonthYearPicker`, no header da `ExpensesTable` (`tableScope`, estado local da página): ele governa a tabela **e** o "para onde o dinheiro vai", que não tem seletor próprio. Clicar num grupo filtra a tabela; clicar numa barra do ritmo move o `tableScope`. "Para onde o dinheiro vai" e compromissos ficam empilhados na coluna esquerda (`.ledgerSide`, ~380px) e a `ExpensesTable` à direita (`.ledgerGrid`, empilha tudo em ≤900px). As alturas são casadas por token, não por flex: as duas listas laterais usam `--side-list-max` e a da tabela `--ledger-max` — mexeu numa, reconfira a outra. O `<select>` de eixo do breakdown precisa de `option { background }` explícito — sem isso o popup nativo do Chrome sai branco com texto claro
- **`components/`** — cards genéricos usados pelas telas de investimentos (`SummaryHeroCard`, `SplitBarCard`, `MonthlyBarsCard` — props opcionais `diverging`/`labelEvery`/`showValues`, cujos defaults preservam o modo original —, `RankedBarsCard`), tabelas (`AssetsTable`, `DividendsTable`; `ExpensesTable` não é tabela — é lista de extrato agrupada por dia, com ícone por grupo via `utils/expenseIcons.tsx` e cabeçalho de dia sticky com total), cards dos gastos (`MonthSummaryCard`, `FixedVariableCard`, `MonthlyPaceCard`, `SpendBreakdownCard`, `CommitmentsCard`) e o resto (`PortfolioActions`, `ExpenseForm`, `MonthStepper`, `GoalCard`). Os cards dos gastos são tipados no domínio de gastos — não reaproveite entre os dois lados
